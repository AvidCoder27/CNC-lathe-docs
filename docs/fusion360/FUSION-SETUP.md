...
Before you read that, take note of a one thing:
...
5. Switch to the `Manufacture` tab in the upper left hand corner.
   1. Switch to the `Turning` along the top of the window.

6. Under the `Setup` drop down (upper left corner), select `New setup`.
   ...
   16. **Save your document** with Ctrl+S. The message doesn't matter.

7. Now, we are going to add some tools.
   ...
   9. Set `Hand` to `L = left handed` and the `Clamping` to `D = rigid lock`. The other defaults are fine. **TODO specify actual holder settings**
   10. Skipping `Setup`, move to the `Cutting data` tab.
       ...
       2. Disable `Use constant surface speed`.
          ...
          3. We do not want to have to change the spindle speed by hand during a cutting operation!
       3. Set the `Spindle Speed` to 10000 rpm (that's 10 thousand!).
   ...
   13. Select the new tool, and hit Ctrl+D to duplicate it.
       ...
       3. Select `OK` to confirm your new, right hand tool.
   14. Hit Escape to exit the tool manager.

8. Now, we will create some toolpaths. Toolpaths have **a lot** of options, so if an option is not explicitly mentioned, just leave it as default.

9. First, we will make a facing operation to remove the stock at the tip.
...
10. Now, we will create a roughing operation.
...
11. Next, we will make a finishing operation.
...
12. We have created our three toolpaths. Now, we will simulate them to check what they will look like and to check for errors.
...
13. Great! We have our toolpaths! The final step of the Fusion process is to post to g-code.
...
You should now have a file called `template.nc` that you can run on the lathe!
